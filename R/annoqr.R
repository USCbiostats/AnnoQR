...
perform_count <- function(q) {
  r <- POST(paste0(host, "/annoq-annotations/_count"), content_type_json(), body = query_obj_to_json(q))
  stop_for_status(r)
...
perform_search_find_count <- function(q) {
  ...
  r <- POST(paste0(host, "/annoq-annotations/_search"), 
            content_type_json(), 
            body = request_body)
  print(r)
...
keywordsQuery <- function(keywords){
  ...
  r <- POST(paste0(host, "/annoq-annotations/_search"), content_type_json(), body = toJSON(body, auto_unbox = T))
  stop_for_status(r)
